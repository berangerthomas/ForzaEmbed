...
def process_item(
    ...
    output_dir: str,
    suffix: str,
    embedding_function: Callable[[list[str]], list],
...
def run_test(
    ...
    Returns:
        tuple[list[np.ndarray], list[np.ndarray]]: Un tuple contenant les embeddings de phrases et les labels.
    """
    ...
    if model_type == "local":
        ...
        for i, item in enumerate(rows, 1):
            try:
                result, embeddings, labels = process_item(
                    item,
                    themes,
                    embed_themes,
                    model_name,
    ...
    return all_phrase_embeddings, all_labels
